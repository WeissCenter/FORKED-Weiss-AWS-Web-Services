            ...
            switch (type) {
                case ValidatorType.HeaderHTML: {
                    ...
                    for (const [index, schema] of headerValidator.schema.entries()) {
                        ...
                        if(!selectValue){
                            errors.push({error: schema.errorText || 'failed to match html file value to required values' + schema.name})
                            continue;
                        }
                        ...
                        switch(schema.type){
                            case 'select':{
                                ...
                                if(Array.isArray(select.value) && !select.value.some((val) => val === selectValue)){
                                    ...
                                    errors.push({error: schema.errorText || 'Failed to match html file value to required value' + schema.name})
                                }
    ...
    switch (schema.type) {
        case "number": {
            ...
            if (typeof select !== 'number') {
                ...
                errors.push({ 'error': schema.errorText || 'Invalid data type for number header', header: schema.name, rule: name });
            }
...
function handleStringSchema(stringSchema: StringSchema, select: string, errors: ValidationError[], schema: Schema, name: string, context?: Record<string, any>) {
    if (stringSchema.array && stringSchema.value?.length) {
        ...
        if (!values.some((value) => select === value)) {
            ...
            errors.push({ 'error': schema.errorText || 'Header value not in the allowed list of values', header: schema.name, rule: name });
        }
    ...
    if (stringSchema.regex) {
        ...
        if (!regexp.test(select)) {
            ...
            errors.push({ 'error': schema.errorText || 'Header string value does not match the regex', header: schema.name, rule: name });
        }
    ...
    if (stringSchema.maxLength && (stringSchema.maxLength === 0 && !select || select.length > stringSchema.maxLength)) {
        ...
        errors.push({ 'error': schema.errorText || 'Header string value exceeds the max length', header: schema.name, rule: name });
    }
}
                ...
                for (const [index, schema] of typeFieldValidator.schema.entries()) {
                    ...
                    switch(schema.type){
                        case 'select':{
                            ...
                            if(Array.isArray(select.value) && !select.value.some((val) => val === selectValue)){
                                errors.push({error: schema.errorText || 'Failed to match file value to required values' + schema.name})
                            }else if (!selectValue.includes(context?.[value.field as string] || '')){
                                errors.push({error: schema.errorText || 'Failed to match file value to required value' + schema.name})
                            }
