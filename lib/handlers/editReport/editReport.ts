...
export const handler: Handler = async (
  ...
  try {
    ...
    const baseReport = await getReportFromDynamo(
      db,
      process.env.REPORT_TABLE || "",
      body.reportID,
      ReportVersion.DRAFT,
      "en",
    );
    ...
    for (const [lang, report] of Object.entries(body.languages)) {
      ...
      const dynamoReports = (await getReportVersionsFromDynamo(
        db,
        process.env.REPORT_TABLE || "",
        body.reportID,
      )) as IReport[];

      let dynamoReportForLang = dynamoReports.find(
        (rpt) => rpt.version === ReportVersion.DRAFT && rpt.lang === lang,
      );
      ...
      const finalizedVersion = dynamoReports.find(
        (rpt) => rpt.version === ReportVersion.FINALIZED,
      );

      if (finalizedVersion)
        await handleFinalizedVersion(db, finalizedVersion, date);

      const [draftResult, draftAudit] = await handleDraftReport(
        db,
        cursor,
        updateReportItem,
        lang,
...
function handleDraftReport(
  ...
  const updateAuditReport = {
    ...
    ...createUpdateItemFromObject(
      { ...body, version: `draft-${body.updated}`, lang },
      ["id", "type"],
    ),
  };
  return Promise.all([
...
async function handleFinalizedVersion(
  db: DynamoDBDocument,
  report: IReport,
  date: number,
) {
  ...
  const updateAuditReport = {
    ...
    ...createUpdateItemFromObject({ ...report, version: `finalized-${date}` }, [
      "id",
      "type",
    ]),
  };
