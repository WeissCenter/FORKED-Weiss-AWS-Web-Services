...
export const handler: Handler = async (
  ...
  try {
    ...
    if (!report && lang !== "en") {
      ...
      reportClone.id = `ID#${reportClone.reportID}#Version#${reportClone.version}#Lang#${lang}`;
...
function getHashForTemplate(filters: Record<string, any>, lang = "en") {
  const filterEntries = Object.entries(filters);

  const mappedFilters = filterEntries.map(([key, value]) => `${key}=${value}`);

  mappedFilters.push(`lang=${lang}`);
